defmodule Treeshake.Utils.BeamParserTest do
  ...
  describe "read/2 - errors" do
    test "returns :error for a non-existent file" do
      assert :error = BeamParser.read("/tmp/does_not_exist_at_all.beam")
    end
  end

  describe "read/2 - module metadata" do
    test "returns the module atom" do
      {:ok, info} = BeamParser.read(beam(DemoApp.Worker))
  ...
  describe "read/2 - abstraction" do
    test "behaviour module has {:behaviour, callbacks}" do
      {:ok, info} = BeamParser.read(beam(DemoApp.Behaviour))
    ...
    test "protocol and behaviour abstractions are distinct tags" do
      {:ok, proto_info} = BeamParser.read(beam(DemoApp.Formatter))
      {:ok, beh_info} = BeamParser.read(beam(DemoApp.Behaviour))
  ...
  describe "read/2 - potential_modules excludes pattern match and guard atoms" do
    test "function-head pattern atom is absent from potential_modules" do
      {:ok, info} = BeamParser.read(beam(DemoApp.PatternMatcher))
  ...
  test "hardcoded MFA tuple {Mod, :fun, arity} appears in calls" do
    {:ok, info} = BeamParser.read("test/fixtures/ebin/Elixir.Task.beam")
  ...
  test "task" do
    assert {:ok, info} =
             Treeshake.Utils.BeamParser.read("test/fixtures/ebin/Elixir.Task.beam")
